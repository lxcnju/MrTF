...
if __name__ == "__main__":
    n_classes = 10
    model = get_basic_net("MLPNet", n_classes, input_size=28, input_channel=3)
    n_params = sum([
        param.numel() for param in model.parameters()
    ]) / 100000.0
    print("Total number of parameters : {}M".format(n_params))

    for net in ["LeNet", "TFCNN", "VGG8", "ResNet8", "ResNet20", "FeMnistNet", "CharLSTM"]:
        model = get_basic_net(net, n_classes, input_size=28, input_channel=3)
        n_params = sum([
            param.numel() for param in model.parameters()
        ]) / 100000.0
        print("Total number of parameters : {}M".format(n_params))
